...
export const StoreResult: React.FC<StoreResultProps> = ({ result }) => {
    ...
    return (
        <div className="w-full flex flex-col gap-5 animate-[float_0.6s_ease-out]">
            ...
            {/* iOS 18.1 Style List Container */}
            <GlassCard className="p-0 overflow-hidden bg-white/95 shadow-lg shadow-black/5">
                <div className="backdrop-blur-xl">
                    <div className="px-5 py-3 border-b border-black/5 bg-[#f8f9fa]/50 flex justify-between items-center">
                        <h3 className="text-[#8e8e93] text-xs font-semibold uppercase tracking-widest">
                            搜尋結果 ({result.stores.length})
                        </h3>
                        <span className="text-[10px] text-[#8e8e93] bg-[#8e8e93]/10 px-2 py-0.5 rounded-full">
                            按距離排序
                        </span>
                    </div>
                            ...
                            return (
                                <div
                                    ...
                                    className={`
                                        ...
                                        ${isSelected ? 'bg-[#007AFF]/5' : 'hover:bg-black/5'}
                                    `}
                                >
                                    {/* Left: Info */}
                                    <div className="flex items-center gap-4 flex-1 min-w-0">
                                        {/* iOS 18.1 Rank Circle */}
                                        <div className={`
                                            flex items-center justify-center w-8 h-8 rounded-full shrink-0 text-sm font-semibold transition-all
                                            ${isSelected
                                        ...
                                        <div className="flex flex-col min-w-0 space-y-0.5">
                                            <h3 className={`text-lg font-semibold truncate pr-2 transition-colors ${isSelected ? 'text-[#007AFF]' : 'text-[#1d1d1f]'}`}>
                                                {store.name}
                                            </h3>
                                            <p className="text-xs text-[#8e8e93] truncate group-hover:text-[#6d6d70] transition-colors">
                                                {store.address}
                                            </p>
                                        </div>
                                    </div>
                                    ...
                                    <div className="flex items-center gap-3 shrink-0 pl-2">
                                        <span className={`font-mono font-semibold text-base whitespace-nowrap transition-colors ${isSelected ? 'text-[#007AFF]' : 'text-[#8e8e93]'}`}>
                                            {store.distance}
                                        </span>
                                        <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor" className={`w-5 h-5 transition-colors ${isSelected ? 'text-[#007AFF]' : 'text-[#c7c7cc]'}`}>
            ...
            {selectedStore && (
                <div className="w-full space-y-4">
                    ...
                    <div className="grid grid-cols-2 gap-3">
                        <GlassCard className="p-4 bg-white/70 shadow-sm">
                            <div className="flex flex-col items-center text-center gap-2">
                                ...
                                <span className="text-sm font-medium text-gray-900 leading-tight">{selectedStore.hours}</span>
                                <span className="text-[10px] text-gray-500 uppercase tracking-wide">營業時間</span>
                            </div>
                        </GlassCard>
                        <GlassCard className="p-4 bg-white/70 shadow-sm">
                            <div className="flex flex-col items-center text-center gap-2">
                                ...
                                <a href={`tel:${selectedStore.phone}`} className="text-sm font-medium text-blue-400 hover:text-blue-300 underline underline-offset-2 leading-tight">
                                    {selectedStore.phone}
                                </a>
                                <span className="text-[10px] text-gray-500 uppercase tracking-wide">聯絡電話</span>
                            </div>
                        </GlassCard>
                    </div>
                    ...
                    <GlassCard className="p-0 overflow-hidden h-[350px] border-gray-200/50 relative group bg-gray-100">
                        ...
                        <div className="absolute top-0 left-0 right-0 z-10 p-4 pointer-events-none">
                            <div className="bg-white/90 backdrop-blur-md px-3 py-1.5 rounded-lg border border-white/50 shadow-sm inline-block">
