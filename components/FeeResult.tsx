...
export const FeeResult: React.FC<FeeResultProps> = ({ data }) => {
    ...
    return (
        <div className="w-full flex flex-col gap-6 animate-[float_0.6s_ease-out]">
            {data.map((storeData) => (
                <GlassCard key={storeData.id} className="p-0 overflow-hidden border-gray-200/50 bg-white/80">
                    {/* Header for the Store Group */}
                    <div className="px-5 py-4 border-b border-white/10 bg-gradient-to-r from-blue-900/40 to-black/40 backdrop-blur-xl">
                        ...
                        <p className="text-gray-500 text-xs mt-1 ml-9">跨區費用價目表</p>
                    </div>

                    {/* Table Header */}
                    <div className="flex bg-white/5 border-b border-white/5 text-[11px] text-white/40 uppercase tracking-widest font-semibold">
                        <div className="w-24 px-4 py-2 text-right border-r border-white/5">跨區費</div>
                    ...
                    <div className="divide-y divide-white/5">
                        {storeData.schedules.map((item, idx) => (
                            <div key={idx} className="flex hover:bg-gray-50 transition-colors group">
                                {/* Price Column */}
                                <div className="w-24 shrink-0 p-4 flex items-center justify-end border-r border-gray-200/50 bg-gray-50/30">
                                    <span className="text-[#32D74B] font-mono font-bold text-base group-hover:scale-105 transition-transform">
                                        {item.price}
                                    </span>
                                </div>
